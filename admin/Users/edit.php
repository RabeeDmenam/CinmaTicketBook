...
if (mysqli_num_rows($op) == 1) {
    ...
    header('Location: ' . url('Users/2index.php'));
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (count($errors) > 0) {
        ...
        $sql = "update admins set name = '$name' , email = '$email',image = '$FinalName' , role_id = $role_id , phone = '$phone' where id = $id";
        $op = mysqli_query($con, $sql);
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="role_id">
                               ...
                               while($roledetails = mysqli_fetch_assoc($roleData)){
                                ?>
                                <option value="<?php echo $roledetails['role_id']; ?>" <?php if ($data['role_id'] == $roledetails['role_id']) {
                                        echo 'selected';
                                    } ?>><?php echo $roledetails['role_name']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
